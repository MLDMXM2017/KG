...
//        addIndexCypher = " CREATE INDEX FOR :`" + domain + "`(name);";
//        loadNodeCypher1 = " CALL{ LOAD CSV FROM '" + csvUrl + "' AS line " + " MERGE (:`" + domain
//                + "` {name:line[0]})}IN TRANSACTIONS;";
//        loadNodeCypher2 = " CALL{ LOAD CSV FROM '" + csvUrl + "' AS line " + " MERGE (:`" + domain
//                + "` {name:line[1]})} IN TRANSACTIONS;";
//        // 拼接生产关系导入cypher
//        String loadRelCypher = null;
//        String type = "RE";
//        loadRelCypher = " CALL { LOAD CSV FROM '" + csvUrl + "' AS line " + " MATCH (m:`" + domain
//                + "`), (n:`" + domain + "`) WHERE m.name = line[0] AND n.name = line[1] " + " MERGE (m)-[r:" + type + "]->(n) "
//                + "	SET r.name = line[2] } IN TRANSACTIONS";
...
//    }
    public void batchInsertByCsv(String domain, String csvUrl, int isCreateIndex) {
        // 构建Cypher语句
        String loadNodeCypher = "USING PERIODIC COMMIT 1000 LOAD CSV WITH HEADERS FROM '" + csvUrl + "' AS line "
                + "MERGE (a:`" + domain + "` {name: COALESCE(line[0], 'undefined'), type: COALESCE(line[3], 'undefined')}) "
                + "MERGE (b:`" + domain + "` {name: COALESCE(line[1], 'undefined'), type: COALESCE(line[4], 'undefined')}) "
                + "CREATE (a)-[:RE {name: COALESCE(line[2], 'undefined'), type: COALESCE(line[5], 'undefined')}]->(b);";

        // 创建索引的Cypher语句
        String addIndexCypher = "CREATE INDEX FOR :`" + domain + "`(name);";
        ...
        // 执行数据导入Cypher语句
        Neo4jUtil.runCypherSql(loadNodeCypher);
    }

    @Override
    public void batchInsertByCsv1(String domain, String csvUrl, int isCreateIndex) {
        String loadNodeCypher1 = null;
        String loadNodeCypher2 = null;
        String addIndexCypher = null;
        addIndexCypher = " CREATE INDEX FOR :`" + domain + "`(name);";
        loadNodeCypher1 = " CALL{ LOAD CSV FROM '" + csvUrl + "' AS line "
                + " MERGE (a:`" + domain + "` {name:COALESCE(line[0], 'undefined'), type:COALESCE(line[3], 'undefined')})} IN TRANSACTIONS;";
        loadNodeCypher2 = " CALL{ LOAD CSV FROM '" + csvUrl + "' AS line "
                + " MERGE (b:`" + domain + "` {name:COALESCE(line[1], 'undefined'), type:COALESCE(line[4], 'undefined')})} IN TRANSACTIONS;";

        // 拼接生产关系导入cypher
        String loadRelCypher = null;
        String type = "RE";
        loadRelCypher = " CALL { LOAD CSV FROM '" + csvUrl + "' AS line "
                + " MATCH (m:`" + domain + "`), (n:`" + domain + "`) WHERE m.name = COALESCE(line[0], 'undefined') AND n.name = COALESCE(line[1], 'undefined') "
                + " MERGE (m)-[r:" + type + "]->(n) "
                + " SET r.name = COALESCE(line[2], 'undefined'), r.type = COALESCE(line[5], 'undefined') } IN TRANSACTIONS";
