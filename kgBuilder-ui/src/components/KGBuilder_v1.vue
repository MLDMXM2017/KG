  ...
  methods: {
    ...
    initGraph() {
      ...
      this.simulation = d3.forceSimulation()
          .force("link", d3.forceLink().id(function(d) { return d.uuid; }).distance(100)) // 调整连接线的长度
          // .force("charge", d3.forceManyBody().strength(-300)) // 调整电荷力
          // .force("center", d3.forceCenter(this.width / 2, this.height / 2)); // 调整中心力
          // .force("link", null)
          // .force("charge", null)
          // .force("center", null);
      ...
      this.simulation.alphaDecay(0.05) // 调整 alpha 的衰减速度
          .alphaMin(0.1); // 设置模拟停止的最小 alpha 值

      this.simulation.on("tick", () => {
        if (this.simulation.alpha() < 0.1) {
          this.simulation.stop(); // 当 alpha 值小于 0.1 时停止模拟
        }
      });
